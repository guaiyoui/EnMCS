...
def run(nNode, query_score_result, graph_list, args, save_result = True):
    ...
    if use_EM:
        ...
        # torch.save(y_pred_list, 'zzz_y_pred_list.pt')
        # exit()
        ...
        labels = 1 - labels
        print("Predicted shape:", y_pred.shape, "Labels shape:", labels.shape)
        f1_score = f1_score_calculation(y_pred.int(), labels.int())
        for i in range(y_pred.shape[0]):
            if torch.equal(torch.where(y_pred[i] == 1)[0], torch.where(labels[i] == 1)[0]):
                print("Query", i, "matches:", torch.equal(torch.where(y_pred[i] == 1)[0], torch.where(labels[i] == 1)[0]))
                print(torch.where(y_pred[i] == 0), torch.where(labels[i] == 0))
